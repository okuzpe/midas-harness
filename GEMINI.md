...
- **Rule: Skill authoring quality gate (always-on)** (`skill-quality.md`, base)
  ...
  - **CHECK:** `manual:` engine-repo diff that adds/changes a user-facing invocable skill without a `docs/skills.md` update in the same change set is a fail; product installs without `docs/skills.md` → `n/a` (see also `change-propagation.md`).
