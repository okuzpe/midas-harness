...
- **Rule: Testing (always-on)** (`testing.md`)
  ...
  - **CHECK:** `manual:` each acceptance-criterion journey has an E2E/API test (or a `/midas-verify` record); for UI journeys the `/midas-verify` record also shows **no uncaught console errors / no failed happy-path network requests** (runtime health, see `verification.md`); an uncovered journey — or a console/network error on the happy path — is a fail.
...
- **Rule: Verification (always-on)** (`verification.md`)
  ...
  - **CHECK:** `manual:` no horizontal overflow on key screens (see `accessibility.md` § layout overflow); `/midas-verify` automates where wired. a hardcoded value not traceable to a `--ds-*` token is a fail.
