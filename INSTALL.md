...
`.harness/`, `harness/state.yaml`), and bumps the `midas_version` stamp so `/midas-doctor` reads it as current:

```bash
npx github:okuzpe/midas-harness#v0.5.10 --update   # pin a version, or omit #vX.Y.Z for the latest main
```
...
`npx github:okuzpe/midas-harness#v0.5.10 --uninstall`.
