...
```bash
npx  github:okuzpe/midas-harness          # canonical .harness/ layout
...
|-----------|----------|----------------|
| **Never installed Midas** | `npx github:okuzpe/midas-harness#v2.0.0-rc.5 --tools=cursor` | `/midas-init` |
...
| Existing 1.x classic/compact/hub | `npx ...#v2.0.0-rc.5 --migrate` then add `--apply` | `/midas-status` |
| Engine refresh on v2 | `npx ...#v2.0.0-rc.5 --update` | `/midas-update` (optional diff-confirm) |
...
## Updating
On a v2 install, **`--update`** refreshes only manifest-owned engine/generated files. It preserves
`.harness/product`, `.harness/rules`, `.harness/runs`, state, MCP, and content outside generated
markers. A modified `vendor` file aborts the update before any write. Adapters re-render for tools in
state. Pass **`--tools=…`** to change the host set and prune unused adapters:

```bash
npx github:okuzpe/midas-harness#v2.0.0-rc.5 --update
npx github:okuzpe/midas-harness#v2.0.0-rc.5 --update --tools=cursor
```
...
```powershell
npx github:okuzpe/midas-harness#v2.0.0-rc.5 --migrate
npx github:okuzpe/midas-harness#v2.0.0-rc.5 --migrate --apply
...
`npx github:okuzpe/midas-harness#v2.0.0-rc.5 --uninstall`.
