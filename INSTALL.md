...
(ask PR|code, then continuous improve; arms Cursor `/loop` — no `--autonomy` required;
distinct from ADR-009 `/midas-auto-sprints`).
...
**Prefer a pinned release** (matches `harness/VERSION` — currently **v2.8.2**):

**macOS / Linux**
```bash
curl -fsSL https://raw.githubusercontent.com/okuzpe/midas-harness/main/install.sh | bash
# shim defaults to github:okuzpe/midas-harness#v2.9.0
```

**Windows (PowerShell)**
```powershell
irm https://raw.githubusercontent.com/okuzpe/midas-harness/main/install.ps1 | iex
# shim defaults to #v2.9.0
```

**Any platform, no shell script** (works with every package manager):
```bash
npx  github:okuzpe/midas-harness#v2.9.0   # recommended — pinned
pnpm dlx github:okuzpe/midas-harness#v2.9.0
bunx github:okuzpe/midas-harness#v2.9.0
```
...
- `--autonomy` — optional bounded control plane (ADR-009). Copies capability to `.harness/autonomy/`
  ...
  After install: `node .harness/autonomy/bin/midas-autopilot.mjs setup` (or `/midas-auto-sprints` in the editor).
...
npx github:okuzpe/midas-harness#v2.9.0 --update --yes             # refresh in CI
...
4. Optional: `/midas-auto-pilot` — start continuous product evolve (asks PR|code; arms Cursor `/loop`; optional
   `cloud` for Automations). Does not require `--autonomy`. Distinct from ADR-009 `/midas-auto-sprints`.
...
|-----------|----------|----------------|
| **Never installed Midas** | `npx github:okuzpe/midas-harness#v2.9.0 --tools=cursor` | `/midas-init` |
...
| Existing 1.x classic/compact/hub | `npx ...#v2.9.0 --migrate` then add `--apply` | `/midas-status` |
| Engine refresh on v2 | `npx ...#v2.9.0 --update` **or** `/midas-update` (pick one) | `/midas-status` when CLI prints `verify: ok` |
...
```bash
npx -y --package=github:okuzpe/midas-harness#v2.9.0 midas --update --tools=cursor
npx -y --package=github:okuzpe/midas-harness#v2.9.0 midas --update --dry-run
```

(`npx github:okuzpe/midas-harness#v2.9.0 --update` alone may fail with `could not determine executable to run`.)
...
```powershell
npx github:okuzpe/midas-harness#v2.9.0 --migrate
npx github:okuzpe/midas-harness#v2.9.0 --migrate --apply
...
`npx github:okuzpe/midas-harness#v2.9.0 --uninstall`.
