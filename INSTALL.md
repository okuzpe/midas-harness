...
**Prefer a pinned release** (matches `harness/VERSION` — currently **v2.10.2**):

**macOS / Linux**
```bash
curl -fsSL https://raw.githubusercontent.com/okuzpe/midas-harness/main/install.sh | bash
# shim defaults to github:okuzpe/midas-harness#v2.10.2
```

**Windows (PowerShell)**
```powershell
irm https://raw.githubusercontent.com/okuzpe/midas-harness/main/install.ps1 | iex
# shim defaults to #v2.10.2
```

**Any platform, no shell script** (works with every package manager):
```bash
npx  github:okuzpe/midas-harness#v2.10.2   # recommended — pinned
pnpm dlx github:okuzpe/midas-harness#v2.10.2
bunx github:okuzpe/midas-harness#v2.10.2
```
...
npx github:okuzpe/midas-harness#v2.10.2 update --yes             # refresh in CI
...
**Any platform (npx):**
```bash
npx github:okuzpe/midas-harness#v2.10.2 update --yes
```
...
|-----------|----------|----------------|
| **Never installed Midas** | `npx github:okuzpe/midas-harness#v2.10.2 --tools=cursor` | `/midas-init` |
...
npx github:okuzpe/midas-harness#v2.10.2 update --resume --yes

# Or undo this run from the installer journal (migrate path restores classic when the snapshot was full):
npx github:okuzpe/midas-harness#v2.10.2 update --rollback --yes
```
...
**`#v2.10.2+`**. If diagnose reports `partial_migrate` (`.harness/product` without engine) and there
...
`npx github:okuzpe/midas-harness#v2.10.2 --tools=cursor` form works on current releases. If npm reports
`could not determine executable to run`, name the bin explicitly:

```bash
npx -y --package=github:okuzpe/midas-harness#v2.10.2 midas --tools=cursor
npx -y --package=github:okuzpe/midas-harness#v2.10.2 midas update --dry-run
```
...
```powershell
npx github:okuzpe/midas-harness#v2.10.2 --migrate
npx github:okuzpe/midas-harness#v2.10.2 --migrate --apply
...
`npx github:okuzpe/midas-harness#v2.10.2 --uninstall`.
