...
> The Midas region is **generated** from `.harness/engine/conventions.md`.
...
All rules in `.harness/engine/conventions.md` plus `.harness/rules/` apply unconditionally. Key points:
...
- **Design system** — all UI uses `.harness/product/design-system.md` tokens; never hardcode colour/spacing/type.
- **Skills portability** — `.harness/engine/skills` is canonical; `.claude/skills` and `.agents/skills`
  are generated discovery mirrors.

Project rules live in `.harness/rules/` and take precedence over base rules with the same slug.

## Path resolution (layout-aware installs)

Read `layout` and `paths` from `.harness/state.yaml` first. Pipeline prose uses `{runs}/` and
`{product}/`; resolve them through `paths.runs` and `paths.product` before any I/O. New writes must
remain under `.harness/` except for generated host adapters.
...
- a **procedure** → a **playbook** in `.harness/product/playbooks/<verb-noun>.md`;
- a **prose preference** → an entry in `.harness/product/conventions.md`.
...
See `.harness/engine/rules/context7-usage.md` for the full rule, the no-tool fallback, and cost guidance.
...
- Side-effecting skills (`/midas-init`, `/define-conventions`, `/start-sprint`, `/close-sprint`, `/midas-doctor`,
  `/midas-adopt`, `/midas-update`, `/midas-verify`, `/midas-design`, `/midas-qa`, `/midas-init --monorepo`, `/midas-tribunal`, `/midas-security-audit`, `/midas-sweep`, `/midas-lean-review`, `/midas-capture`, `/midas-align`, `/midas-bundle`, `/midas-progress`, `/midas-explore`, `/midas-autopilot`, `/midas-improve-loop`, `/midas-retro`) are **user-typed slash commands**
