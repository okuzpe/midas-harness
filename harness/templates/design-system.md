<!-- Phase 5 — Design System artifact. Orchestrator defines; build tier writes.
     ...
     adapters rendered. Inherits the base token layer from <paths.engine>/design-system/tokens.{json,css}
...
(`<paths.engine>/design-system/tokens.json` / `tokens.css`). Phase 5 writes project-owned token files
to `{product}/design-system/tokens.{json,css}`; the engine base remains immutable.
...
<!-- List only tokens that DIFFER from <paths.engine>/design-system/tokens.json. -->
