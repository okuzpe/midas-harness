#!/usr/bin/env node
// doctor.mjs — Midas adapter drift checker (dependency-free, Node ESM).
//
...
function main() {
  ...
  console.log('midas doctor: checking generated adapters against harness/conventions.md');
  ...
  if (drift) {
    console.log('\nAdapters are OUT OF SYNC. Run `node scripts/doctor.mjs --fix` (or `/midas-doctor`) to re-render.');
    process.exit(1);
  }
