...
function readTokens(root) {
  const path = join(root, 'harness', 'design-system', 'tokens.json');
...
export function renderDesignSystemTokens(root = ROOT) {
  ...
  const path = join(root, 'harness', 'design-system', 'tokens.css');
  const before = existsSync(path) ? readFileSync(path, 'utf8') : '';
  if (before === css) return { path: 'harness/design-system/tokens.css', status: 'unchanged' };
  writeFileSync(path, css, 'utf8');
  return { path: 'harness/design-system/tokens.css', status: 'written' };
}
