...
### Step 1 — Codebase inventory (scout)
...
Context7), test setup, CI, and any existing `AGENTS.md` / `CLAUDE.md` / `.cursor` / `.windsurf`. Write
...
### Step 4 — Backfill product context (document-existing mode)
Optionally run idea/market/business in "document-existing" mode: record the project's *actual* purpose,
audience, and constraints (and links to existing docs) rather than inventing them. Skipped or
backfilled gates carry a **recorded assumption** in `STATE.md` and an honest `entry_stage`.
...
Set `harness/state.yaml`: `mode: brownfield`, `entry_stage` (typically `tech_architecture` or
`architecture_rules`), recorded assumptions for skipped gates.
