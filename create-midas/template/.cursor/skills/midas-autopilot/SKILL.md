...
description: "Bounded sprint autopilot — thin guide to midas-autopilot setup/status/dry-run/tick. Use when enabling ADR-009 autonomy, checking blockers, or scheduling one task per tick. Does not auto-run tick or cursor-cloud from chat. For continuous improve without a checklist, use /midas-improve-loop."
metadata:
...
---
# midas-autopilot — sprint checklist actuator (ADR-009 CLI)
...
- Blocker `no_code_task` → open items are operator/manual. Activate a **code** sprint, **or** discover improvements via `/midas-improve-loop` then return here for policy-gated `tick`s.
- User wants continuous product-aligned improve without a code checklist yet → `/midas-improve-loop`.
- User wants a raw chat-only loop without Midas caps → Cursor `/loop`; prefer `/midas-improve-loop` when product context should drive picks.
...
Do **not** ask the user to export `MIDAS_AUTONOMY_AUTHZ_KEY` for everyday local use. Env override remains valid for CI. Exit `0` with `status: configured` means authz is fine but the sprint has no code tasks — next step is `/start-sprint` or `/midas-improve-loop`, not re-setup.
...
`--runner=cursor-cloud` needs `CURSOR_API_KEY` (Cloud Agents). Local overnight without that key = `fake` pilot or `/midas-improve-loop` — not this CLI cloud path.
