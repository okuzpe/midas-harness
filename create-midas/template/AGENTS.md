...
Read `layout` and `paths` from the state file first (`paths.state` in YAML — classic: `harness/state.yaml`, compact: `.midas/state.yaml`). Pipeline prose uses `{runs}/` as a token for the runs base. Substitute `{runs}` with `paths.runs` before writing audits, verifications, sprints, or sweeps. Engine source is at `paths.engine` (`harness/` or `.midas/engine/`).
...
- Side-effecting skills (`/midas-init`, `/define-conventions`, `/start-sprint`, `/close-sprint`, `/midas-doctor`,
  `/midas-adopt`, `/midas-update`, `/midas-verify`, `/midas-monorepo`, `/midas-tribunal`, `/midas-security-audit`, `/midas-sweep`, `/midas-capture`, `/midas-align`, `/midas-bundle`, `/midas-progress`) are **user-typed slash commands**
