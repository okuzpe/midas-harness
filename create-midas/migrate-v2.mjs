...
const PRODUCT_DIRS = ['adr', 'playbooks', 'sprints'];
...
function knownProductPaths(root, productRoot, stateRaw) {
  ...
  const pathKeys = new Set(['artifact', 'artifacts', 'evidence', 'file', 'output', 'path', 'record', 'report', 'sprint']);
  for (const line of stateRaw.split(/\r?\n/)) {
    const match = line.match(/^\s*(?:-\s*)?([A-Za-z_][\w-]*):\s*['"]?([^'"]+)['"]?\s*(?:#.*)?$/);
    if (!match || !pathKeys.has(match[1])) continue;
    const value = match[2].trim().replace(/[),\]]+$/, '');
    const prefixes = [posix(productRoot) + '/', '{product}/'];
...
function rewriteState(raw) {
  ...
  const rewriteKnownPath = (value) => {
    ...
    if (value === '.midas/engine/rules') return '.harness/engine/rules';
    if (value.startsWith('.midas/engine/rules/')) return `.harness/engine/rules/${value.slice('.midas/engine/rules/'.length)}`;
  ...
  return next.split(/\r?\n/).map((line) => {
    ...
    if (!match || !pathKeys.has(match[2])) return line;
    const rewritten = rewriteKnownPath(match[4].trim());
    return rewritten === match[4].trim()
      ? line
      : `${match[1]}${match[2]}: ${match[3]}${rewritten}${match[3]}${match[5]}`;
  }).join('\n');
}
