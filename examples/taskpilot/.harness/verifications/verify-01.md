...
| 2 | Login → 200 + cookie | playwright-mcp | `getByLabel('Email')`, `getByRole('button',{name:'Sign in'})` | redirect to `/board`, cookie set | redirected, cookie set | PASS | `verify-01/login-success.png` |
...
| 9 | Unauthenticated page → middleware redirect to `/login` | playwright-mcp | nav `/board` (no cookie) | 302 → `/login?next=/board` | redirected | PASS | `verify-01/redirect-login.png` |
