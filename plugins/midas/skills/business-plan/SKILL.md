...
6. **Go / no-go** — render an explicit recommendation with rationale, then obtain **human sign-off**
   via `AskUserQuestion`. Record the decision + who signed off + the date in the document and in
   `STATE.md`. On "no-go", stop the lifecycle and log why.
7. **Write `product/business-plan.md`** from `harness/templates/business-plan.md`; update
