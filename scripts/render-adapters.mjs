#!/usr/bin/env node
// render-adapters.mjs — Keel adapter renderer (dependency-free, Node ESM).
//
...
//   - CLAUDE.md                 -> keeps content outside markers; managed block holds the Keel note.
//                                  Guarantees an `@AGENTS.md` import sits above the managed block.
//   - .cursor/rules/00-keel.mdc -> Cursor frontmatter FIRST (file head), then a managed body.
//   - .windsurf/rules/00-keel.md-> Windsurf frontmatter FIRST (file head), then a managed body.
//
...
//
// It also writes a content hash to .harness/adapters.hash so /keel-doctor can detect drift.
...
const BEGIN = '<!-- keel:begin GENERATED — edit harness/conventions.md, run keel-doctor -->';
const END = '<!-- keel:end -->';
...
export function computeAdapters(root = ROOT) {
  ...
  // --- CLAUDE.md: managed block holds ONLY the Keel note (no @AGENTS.md — that import lives
  //     above the markers so it is never duplicated). Guarantee the import exists.
  const claudeInner = [
    '## Keel (generated — edit harness/conventions.md, run keel-doctor)',
    '- Source of truth: `harness/`, `.claude/skills/`, `AGENTS.md`. This block is rendered by',
    '  `scripts/render-adapters.mjs`; do not hand-edit between the keel markers.',
  ...
  const sharedBody = [
    BEGIN,
    '> Generated by Keel from `harness/conventions.md`. Do not hand-edit — run `/keel-doctor`',
  ...
  // --- .cursor/rules/00-keel.mdc: frontmatter FIRST, then managed body -------------------------
  const cursorContent =
    '---\n' +
    'description: Keel base conventions (always-on project law). Generated from harness/conventions.md.\n' +
    'globs:\n' +
    'alwaysApply: true\n' +
    '---\n\n' +
    sharedBody;
  const cursorAbs = join(root, '.cursor', 'rules', '00-keel.mdc');

  // --- .windsurf/rules/00-keel.md: frontmatter FIRST, then managed body ------------------------
  const windsurfContent =
    '---\n' +
    'trigger: always_on\n' +
    '---\n\n' +
    sharedBody;
  const windsurfAbs = join(root, '.windsurf', 'rules', '00-keel.md');
  ...
  return {
    ...
    files: [
      ...
      { path: '.cursor/rules/00-keel.mdc', abs: cursorAbs, content: cursorContent },
      { path: '.windsurf/rules/00-keel.md', abs: windsurfAbs, content: windsurfContent },
    ],
  };
}
...
if (invokedDirectly) {
  ...
  console.log('keel render-adapters: rendered from harness/conventions.md');
