// portable-skills.mjs — generate `.agents/skills` from `.claude/skills`.
//
// The source of truth stays in `.claude/skills`. This helper materializes the portable mirror used
// by hosts that discover skills at `.agents/skills`.
...
function renderPortableSkillFile(sourceFile, targetFile, sourceRoot) {
  ...
  if (!parsed.name || !parsed.description) {
    throw new Error(`Portable skill frontmatter missing name/description: ${sourceFile}`);
  }
  const rel = sourceFile.slice(sourceRoot.length + 1).replace(/\\/g, '/');
  const portable = renderPortableFrontmatter(rel, parsed) + body.replace(/^\r?\n/, '');
  mkdirSync(dirname(targetFile), { recursive: true });
  writeFileSync(targetFile, portable.endsWith('\n') ? portable : `${portable}\n`, 'utf8');
}
...
/**
 * Render `.agents/skills` from `.claude/skills`.
 * Returns a list of written file paths relative to `root`.
 */
export function renderPortableSkillsTree(root, { sourceDir = '.claude/skills', targetDir = '.agents/skills' } = {}) {
