...
const FROZEN_RUNS = ['audits', 'verifications', 'debates', 'sprints', 'sweeps', 'lean', 'retros', 'investigate', 'auto-pilot'];
...
export function toCanonical(rel, layout) {
  const p = posix(rel);
  if (layout === 'harness') {
    if (p === '.harness/state.yaml') return 'harness/state.yaml';
    if (p === '.harness/product') return 'product';
    if (p.startsWith('.harness/product/')) return 'product' + p.slice('.harness/product'.length);
    if (p === '.harness/rules') return 'harness/rules';
    if (p.startsWith('.harness/rules/')) return 'harness/rules' + p.slice('.harness/rules'.length);
...
export function fromCanonical(canonical, layout) {
  ...
  if (layout === 'harness') {
    if (p === 'harness/state.yaml') return '.harness/state.yaml';
    if (p === 'product') return '.harness/product';
    if (p.startsWith('product/')) return '.harness/product' + p.slice('product'.length);
    if (p === 'harness/rules') return '.harness/rules';
    if (p.startsWith('harness/rules/')) return '.harness/rules' + p.slice('harness/rules'.length);
...
/** Repo-relative runs subtree (honours `paths.runs` overrides in state). */
function runsRel(root, layout, sub = '') {
  const p = resolvePaths(root, layout);
  return sub ? posix(join(p.runs, sub)) : p.runs;
}
...
export function collectRecallPaths(stateYaml, root, layout) {
  ...
  if (activeId) {
    ...
    const progressRel = join(runsRel(root, layout, 'sprints'), `${activeId}-progress.md`);
    if (existsSync(join(root, progressRel))) paths.add(`.harness/sprints/${activeId}-progress.md`);
  }
  ...
  if (mode === 'brownfield') {
    ...
    const sweeps = listUnder(root, runsRel(root, layout, 'sweeps'));
  ...
  if (only?.length) {
    ...
    if (profile === 'recall') {
      ...
      if (wantsKnowledge || wantsMemory) {
        ...
        for (const f of listUnder(root, fromCanonical('product/adr', layout))) add(toCanonical(f, layout));
        for (const f of listUnder(root, fromCanonical('product/playbooks', layout))) add(toCanonical(f, layout));
        for (const f of listUnder(root, fromCanonical('product/sprints', layout))) add(toCanonical(f, layout));
      }
      if (wantsMemory) {
        for (const p of collectProjectRulePaths(stateYaml, root, layout)) add(p);
        for (const f of listUnder(root, runsRel(root, layout, 'sprints'))) add(toCanonical(f, layout));
      }
      if (wantsEvidence) {
        for (const sub of FROZEN_RUNS) {
          for (const f of listUnder(root, runsRel(root, layout, sub))) add(toCanonical(f, layout));
        }
      }
    }
    if (includeTests) {
