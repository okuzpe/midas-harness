...
- **Pin a release for a reproducible, reviewable install.** The shims and `npx` resolve from the
  ...
  `npx github:okuzpe/midas-harness#v0.5.20`.
...
| SEC-005 | `curl\|bash` / `irm\|iex` installers pipe a remote script from the mutable `main` branch | Documented (§Installing Midas securely); `npx` and pinned-tag (`#v0.5.20`) alternatives provided |
