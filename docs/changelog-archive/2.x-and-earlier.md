# Changelog archive (2.x and earlier)

Moved from root CHANGELOG.md at the 3.0.0 cut.
...
- **Split `scripts/test.mjs` by domain** (audit M5 / U7) shipped in 3.0.0 — thin `scripts/test.mjs` plus `scripts/test/{json,skills,adapters,version,gates,installer,paths,bundle,runtime}.mjs`. `node scripts/test.mjs` stays the single test entry.
