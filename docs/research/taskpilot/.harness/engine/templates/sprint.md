<!-- Phase 6 — Sprint artifact. Created by /plan-sprints (Phase 6 planning).
     ...
     Phase 8 audit checks DoD conformance against effective base/project rules (do not self-grade). -->
...
<!-- TODO: sprint-specific checklist referencing the rules that apply.
     Standard items below — add stack-specific items from <paths.rules>/. -->

- [ ] All acceptance criteria above are met.
- [ ] Every changed behaviour has a passing automated test (`<paths.engine>/rules/testing.md` plus project overlays).
- [ ] No lint errors or type errors (`<paths.engine>/rules/code-quality.md` plus project overlays).
- [ ] Code reviewed (at least one other agent tier or human).
- [ ] No new hardcoded colours/spacing/type sizes (design-system rule).
- [ ] Context7 was used for every new third-party API touched (`<paths.engine>/rules/context7-usage.md`).
- [ ] `paths.state` updated (`stage_status: gate_pending` when last task done).
