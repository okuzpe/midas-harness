...
Midas stores **long-term** project truth in `{product}/*`, `<paths.rules>/*`, and frozen `{runs}/*`
...
|---|---|---|---|
| 0 | Idea Intake | `pipeline/0-idea-intake.md` | `{product}/idea.md`, `paths.state` |
...
[`pipeline/lite.md`](./pipeline/lite.md). `/midas-status` prints `Track: lite|full` and never
recommends `/market-research` as Next on lite. `{product}/business-plan.md` is still written (thin
...
ladder in `<paths.engine>/rules/verification.md`: static → tests → runtime smoke →
