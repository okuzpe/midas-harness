...
| Authz HMAC | Local `.harness/autonomy/authz/hmac` (auto via `setup`, gitignored) or optional `MIDAS_AUTONOMY_AUTHZ_KEY`; builders denied `authz/**` via fail-closed hooks; `schema_version: 2`; unsigned/v1 grants fail closed |
