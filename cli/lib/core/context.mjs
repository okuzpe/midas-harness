...
/**
 * True when `dir` is the midas-harness **engine repository** (authored source), not a product
 * install target. Product installs must never nest `.harness/engine` here — that is biting the
 * tail (engine-on-engine). Markers: authored `harness/VERSION` + `cli/` + build script.
 * @param {string} dir
 */
export function isMidasEngineRepository(dir) {
  return (
    existsSync(join(dir, 'harness', 'VERSION')) &&
    existsSync(join(dir, 'cli', 'package.json')) &&
