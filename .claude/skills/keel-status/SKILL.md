...
# keel-status — where am I, what's next
...
1. **Read `harness/state.yaml`.** If it is missing, report that Keel is not initialized and direct the
   user to `/keel-init`. If it exists but does not parse, say so plainly and point at `/keel-doctor`.
...
```
Keel · <name> · <mode> · profile <cost_profile>
...
Mention `/keel-doctor` only if you observed adapter or config drift while reading.
